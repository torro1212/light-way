...
const values = [
  {
    ...
    description: 'זו לא מכירה, זו הזמנה. מגיעים לכאן כי משהו בפנים אמר "כן", לא בגלל שכנוע.',
    ...
    >
      ...
      <div className="container-custom relative z-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-10">
          {values.map((value, index) => (
            ...
            >
              ...
              <div className="relative h-full p-8 md:p-10 rounded-[2rem] md:rounded-[2.5rem] bg-white/80 backdrop-blur-xl border-2 border-primary/20 group-hover:border-primary/40 transition-all duration-500 shadow-soft hover:shadow-2xl hover:-translate-y-4">
                {/* Icon Container */}
                <div className="relative w-16 h-16 md:w-20 md:h-20 mb-8 md:mb-10">
                  <div className="absolute inset-0 bg-primary/10 rounded-xl md:rounded-2xl rotate-6 group-hover:rotate-12 transition-transform duration-500" />
                  <div className="relative w-full h-full rounded-xl md:rounded-2xl bg-white border-2 border-primary/20 flex items-center justify-center shadow-lg group-hover:-rotate-3 transition-transform duration-500">
                    <value.icon className="w-8 h-8 md:w-10 md:h-10 text-primary transition-transform duration-500 group-hover:scale-110" />
                  </div>
                </div>

                <h3 className="font-display text-xl md:text-2xl font-bold text-foreground/95 mb-4 group-hover:text-primary transition-colors drop-shadow-sm">
                  {value.title}
                </h3>
                <p className="text-base md:text-lg text-foreground/85 leading-relaxed font-body font-medium">
                  {value.description}
                </p>
              </div>
            </div>
        ...
        <div className="mt-24 md:mt-48 max-w-5xl mx-auto">
          <div className="relative p-6 sm:p-8 md:p-12 lg:p-16 xl:p-24 rounded-[2.5rem] md:rounded-[3rem] lg:rounded-[4rem] overflow-hidden group shadow-2xl border-2 border-primary/30" style={{ background: 'linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 50%, #1a1a1a 100%)' }}>
            {/* Animated Background Overlay */}
            <div 
              className="absolute top-0 right-0 w-[30rem] md:w-[40rem] h-[30rem] md:h-[40rem] bg-primary/30 rounded-full blur-[100px] md:blur-[120px] transition-transform duration-1000 group-hover:scale-125"
              style={{ transform: `translate(${mousePos.x * 50}px, ${mousePos.y * 50}px)` }}
            />
            
            <Quote className="w-12 h-12 sm:w-16 sm:h-16 md:w-24 md:h-24 text-primary/25 absolute top-6 right-6 sm:top-8 sm:right-8 md:top-12 md:right-12 animate-float drop-shadow-lg" />
            
            <div className="relative z-10 space-y-6 sm:space-y-8 md:space-y-12 text-center px-2 sm:px-4 md:px-6">
              <p className="font-display text-lg sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl 2xl:text-6xl italic leading-relaxed font-bold text-white drop-shadow-lg">
              ...
              <div className="flex flex-col items-center gap-3 sm:gap-4 md:gap-6">
                <div className="w-10 sm:w-12 md:w-16 h-px bg-primary/60" />
                <div className="flex flex-col items-center">
                  <span className="text-primary font-body text-xs sm:text-sm md:text-base lg:text-lg font-extrabold uppercase tracking-[0.2em] sm:tracking-[0.3em] md:tracking-[0.5em] mb-1 md:mb-2 drop-shadow-md">אתי אסתר דיין</span>
                  <span className="text-white/80 font-body text-[10px] sm:text-xs md:text-sm lg:text-base font-semibold uppercase tracking-[0.15em] sm:tracking-[0.2em] drop-shadow-md">מנחת מסעות ריפוי</span>
                </div>
              </div>
            </div>
          </div>
